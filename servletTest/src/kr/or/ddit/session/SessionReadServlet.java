...
public class SessionReadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//		==> 이 메서드의 반환값은 Object형이라 형변환을 해서 사용해야 한다.
		...
		//		==> Session에 저장된 모든 세션의 'key값'들을 반환한다.
		...
		// 세션ID ==> 세션을 구분하기위한 고유한 값(서버에 의해 자동 생성된다.)
		out.println("세션 ID : " + session.getId() + "<br>");
		
		// 세션생성시간 ==> 1970년 1월 1일부터 세션이 만들어진 날짜 및 시간까지의 경과시간(단위 : 밀리세컨드)
		out.println("세션 생성 시간 : " + session.getCreationTime() + "<br>");
		
		// 최근 세션 접근시간 ==> 1970년1월1일부터 경과시간(밀리세컨드 단위)
		out.println("최근 세션 접근 시간 : " + session.getLastAccessedTime() + "<br>");
		
		// 세션 유효시간 (초 단위) ==> '최근 세션 접근 시간'이후로 '세션 유효시간'이 경과하면 세션이
		//							자동으로 삭제된다.(기본값 : 30분)
