...
public class RequestTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 요청할 때 넘어온 값을 가져오기
		// request.getParmeter("파라미터명")
		//			==> 해당 파라미터에 설정될 '값'을 가져온다.(가져온 값은 'String'형 이다.)
		// jsp?에 있는 input에 있는 name값과 같다. 즉 태그의 name값이 파라미터명이다.
		// 요청 받아올때 한글이 깨지는데 그 이유는 요청 받아올때 utf-8로 설정안해줘서 그렇다
		String userName = request.getParameter("username");
		String job = request.getParameter("job");
		
		// request.getParameterValues("파라미터명")
		//			==> 전달되는 '파라미터명'이 여러개일 경우에는 사용한다.
		...
		out.println("<table border='1'>"
				+ "<tr><td>파라미터 이름</td><td>파라미터 value값</td></tr>");
		Map<String, String[]> paramMap = request.getParameterMap();
		for(String paramName : paramMap.keySet()) {
			
			out.println("<tr><td>" + paramName + "</td>");	// 파라미터 name출력
			out.println("<td>");
			...
			if(paramValues==null || paramValues.length==0) {	// 파라미터가 없는 경우
				continue;
			} else if(paramValues.length==1) {	// 파라미터가 배열이 아닌 경우
