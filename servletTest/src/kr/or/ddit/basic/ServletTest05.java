...
public class ServletTest05 extends HttpServlet {
	...
	/*
		초기화 파라미터
		- 처음 WAS가 기동될 때 읽어 들이는 파라미터 값들을 초기화 파라미터라고 한다.
		- '서블릿 초기화 파라미터'와 '컨텍스트 초기화 파라미터'가 있다.
		- '서블릿 초기화 파라미터'는 해당 서블릿에서만 사용 가능하고,
		  '컨텍스트 초기화 파라미터'는 전체 웹 어플리케이션 전체 영역에서 사용 가능하다.
		
		1. 서블릿 초기화 파라미터 
			==> web.xml에 설정하는 방법과 애너테이션에 설정하는 방법 2가지가 있다.
	*/
		// 서블릿 초기화 파라미터값을 구하기
